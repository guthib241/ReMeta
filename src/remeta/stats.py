...
from .model import DataError, MetaAnalysis, RATIO_MEASURES, Study, TABLE_MEASURES
...
def effect_size(study: Study, measure: str, correction: float = 0.5) -> tuple[float, float]:
    ...
    if not study.has_table:
        raise DataError(f"study {study.id!r}: no usable data")

    if measure not in TABLE_MEASURES:
        raise DataError(
            f"study {study.id!r}: {measure} cannot be derived from a 2x2 table of "
            f"counts (only {', '.join(sorted(TABLE_MEASURES))} can). Supply a "
...
def pool(
    ...
    pairs = [(s.id, *effect_size(s, measure)) for s in studies]
...
def pool_meta(ma: MetaAnalysis, studies: list[Study] | None = None) -> PooledResult:
